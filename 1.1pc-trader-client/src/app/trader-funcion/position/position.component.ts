...
export class PositionComponent implements OnInit,OnDestroy {
	...
	positionListJson = {};
	ngOnInit() {
		this._socket.addPosition();
		this.connect = this._socket.getPositionTotal().subscribe(res => {
			let data = JSON.parse(res.toString());
			if(data.content.length == 0){
				delete this.positionListJson[data.productId];
				return;
			}
			...
			if(this.positionListJson[data.productId] == undefined){
				this.positionListJson[data.productId] = data.content;
			}
			else if(this.positionListJson[data.productId][1] == undefined && data.content[1] != undefined){
				this.positionListJson[data.productId] = data.content;
			}
			else if(this.positionListJson[data.productId][1] != undefined && data.content[1] == undefined){
				this.positionListJson[data.productId] = data.content;
				...
				if(
					this.positionListJson[data.productId][0].toNewPrice != data.content[0].toNewPrice || 
					this.positionListJson[data.productId][0].positionVolume != data.content[0].positionVolume
				)
				{
					this.positionListJson[data.productId] = data.content;
				}
				else if(this.positionListJson[data.productId][1] != undefined){
					if(
						this.positionListJson[data.productId][1].toNewPrice != data.content[1].toNewPrice || 
						this.positionListJson[data.productId][1].positionVolume != data.content[1].positionVolume
					)
					{
						this.positionListJson[data.productId] = data.content;
					}
	...
	ngOnDestroy() {
		this._socket.rejucePosition();
		this.connect.unsubscribe();
	}
	/* 平仓 */
	closePosition():void {
		if(this.chooseProId == '' || this.choosePositionIndex == -1){
			return;
		}
		let json = [
		...
		Window._confirm(json,function(){
			let body = {"orderRequestVIce":{"productId":self.chooseProId,"orderDirect":self.choosePositionDir}}
	...
	quicklyClosePosition(){
		...
		let body = {"orderRequestVIce":{"productId":this.chooseProId,"orderDirect":this.choosePositionDir}}
		let self = this;
	...
	positionChoose(id,index,proName,dir,vol,price,positionPrice):void {
